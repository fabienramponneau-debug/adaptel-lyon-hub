...
interface Action {
  ...
  contact_vu?: string | null; // NOUVEAU : personne vue
}
...
export function ActionTimeline({ actions, onActionClick }: ActionTimelineProps) {
  ...
  const futureActions = actions
    .filter((a) => new Date(a.date_action) >= now)
    .sort(
      (a, b) =>
  ...
  const renderAction = (action: Action, isPast: boolean) => (
    ...
    >
      <div
        className={`absolute left-0 top-2 w-6 h-6 rounded-full border-2 flex items-center justify-center ${getActionColor(
          action.type
        )} ${isPast ? "opacity-60" : ""}`}
      >
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            {format(new Date(action.date_action), "d MMMM yyyy", {
              locale: fr,
            })}
          </span>
          {action.statut_action === "a_relancer" && action.relance_date && (
            ...
            >
              Relance:{" "}
              {format(new Date(action.relance_date), "d MMM", { locale: fr })}
            </Badge>
          )}
        </div>

        {/* NOUVEAU : contact vu pour visite / rdv */}
        {action.contact_vu && (action.type === "visite" || action.type === "rdv") && (
          <p
          ...
          >
            ...
            <span>Personne vue : {action.contact_vu}</span>
          </p>
        )}
